...
Мы будем изучать пакет arrey для работы с таблицами (рис. [-@fig:001] ) 

![Программа 1](image/1.png){ #fig:001 width=70% }

 
Если в столбце таблицы много текста, у нас могут возникнуть проблемы с его корректным отображением с помощью только l, c и r. (рис. [-@fig:002] ) Как можно заметить, текст не отображается полностью и уходит за пределы листа.

![Программа 2](image/2.png){ #fig:002 width=70% }

Изменим программу согласно лабораторной работе и запустим снова (рис. [-@fig:003] ) 
...
Пакет booktabs предоставляет четыре различных типа линий. Каждую из этих команд нужно использовать в начале строки или после другой команды. (рис. [-@fig:005] ) 
...
Вертикальное объединение ячеек не поддерживается в LaTeX. Обычно достаточно оставить ячейки пустыми, как в примере: (рис. [-@fig:008] )
...
Продемонстрируем как можно добавить вертикальную линию (рис. [-@fig:011] )
